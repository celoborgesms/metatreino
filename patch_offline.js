...
const anchor='    auth.onAuthStateChanged(function(user) {';
const helpers=`    function usuarioOfflineMinimo(user) {
      if (!user) return null;
...
`;
if (!s.includes('function usuarioOfflineMinimo')) s=s.replace(anchor, helpers+anchor);
...
s=s.replace('      bloquearApp("Não foi possível verificar o login agora. Tente novamente em alguns instantes.");','      tentarEntrarOffline("Não foi possível verificar o login agora.");');
